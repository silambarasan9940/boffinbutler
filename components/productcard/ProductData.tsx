...
const ProductData: React.FC<ProductDataProps> = ({
  ...
  const handleClick = () => {
    ...
    console.log(_source.url_key, "url source");
  };
    ...
    <>
      <div
        className={`${isProductPadding ? "p-4" : "p-1"} ${
          ...
          isProductShadow ? "shadow-lg" : ""
        } rounded-xl text-left min-h-80 cursor-pointer`}
        onClick={handleClick}
      >
        <img
          src={`${imageUrl}catalog/product${
            _source?.image ? _source?.image : "/0/0/008-1.jpg"
          }`}
          alt={name}
        ...
        <div className="relative group">
          <h2
            ...
            onClick={handleClick}
          >
            {_source?.name.length < 27
              ? _source?.name
              : _source?.name.substring(0, 24) + "..."}
          </h2>
